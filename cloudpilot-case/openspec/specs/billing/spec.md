...
定义报价计算与项目成本汇总（Supporting 子域）的规范基线：实时报价、报价快照、成本汇总与 ACL 防腐，来源于 DDD 模型 BillingContext 与 design 关键决策 D1-D4，供 P5 代码桥接与测试映射使用。编号体系：IV-N 来自 DDD 聚合不变量（03 §III）；B-N 为从 design 关键决策派生的可测约束（见 ADR-005）。
## Requirements
### Requirement: 实时报价
