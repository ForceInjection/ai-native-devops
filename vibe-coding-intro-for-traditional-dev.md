...
A2A 是 **Agent 之间通信协作的协议**，由 Google 于 2025 年 4 月发布。当一个任务需要多个 Agent 分工完成时，A2A 定义它们如何交换信息、传递任务、协调结果。
...
**对照编辑器中的 Skill 实现**：Claude Code 中由 `SKILL.md` 定义的自定义指令（如 `/pdf`、`/opsx:propose`），本质就是一个 Skill——一段提示词 + 一组步骤，触发后按预定流程跑完。注意与 `/commit`、`/review-pr` 等编辑器内置指令区分：前者可自定义，后者固化在编辑器中。
...
| 概念                    | 市场上可见的对应工具                                                                            | 说明                                                                                     |
| :---------------------- | :---------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------- |
| **AI Agent**            | Cursor Agent、Claude Code、Devin、GitHub Copilot Workspace                                      | 能自主规划任务、调用工具、迭代修复的 IDE/CLI 形态 Agent                                  |
...
| **Skill**               | Claude Code 的自定义 Skill（如 `SKILL.md` 定义的 `/pdf`、`/opsx:propose`）、Qoder slash command | 可复用的固定流程，由提示词 + 步骤定义构成                                                |
...
> 以下内容浓缩自 [`./ai-native-devops/ai-native-devops.md`](./ai-native-devops/ai-native-devops.md)，聚焦"AI 如何增强 DevOps 全流程"。
...
> RACI 字母含义：**A**=Accountable（最终负责，唯一）、**R**=Responsible（执行）、**C**=Consulted（咨询）、**I**=Informed（知会）。
...
> 与本节设计原则衔接的现有示例：[`./ai-native-devops/ai-native-devops-sample-change-walkthrough.md`](./ai-native-devops/ai-native-devops-sample-change-walkthrough.md)（订单取消能力变更全链路演练）。
...
| **资源范围（mock）** | ECS、RDS、OSS、Redis、SLB 五类虚拟资源              |
...
| §5 功能需求     | FR-01~FR-11，P0 核心 8 项 + P1 增强 2 项 + P2 后续迭代 1 项                |
...
> IV-1 ~ IV-8 全部通过 `@ddd-model-review` 校验（完整性 95% 因 FR-11 到期告警为后续迭代），直接桥接到 §2.4.5 的 `specs/<context>/spec.md`——每个 IV-N 至少对应一个 `#### Scenario:` 块（实测覆盖率 8/8，可用 `grep -rE 'IV-[1-8]' cloudpilot-case/04-openspec/specs/` 复核）。
>
> **为什么用 Skill 流水线而不是手工建模**：传统 DDD 实施常见痛点是"事件风暴一次性产出、之后再无回溯触发"。`ddd-*` Skill 主干显式定义了双向反馈条件（如不变量表达率低于约定阈值时回到 `@ddd-aggregates`，具体阈值由 [`@ddd-model-review`](https://github.com/ForceInjection/domain-driven-design-skills/blob/main/skills/ddd-model-review/SKILL.md) 维护），让建模从**线性交付**转为**可观测的闭环迭代**——这正是 AI-Native DevOps 在建模阶段的具象体现。
...
| 能力         | 真实实现       | Mock 实现                                                                              |
| :----------- | :------------- | :------------------------------------------------------------------------------------- |
| 创建云主机   | 调阿里/AWS API | 插入内存表 + 可配置延迟（默认 5–30 分钟，UI 演示压缩到 5–30 秒）后状态转 `PROVISIONED` |
| 查询资源状态 | 调云控制台     | 读内存表                                                                               |
| 报价         | 调计费服务     | 查静态 YAML 价格表                                                                     |
| 释放资源     | 调云 API       | 状态置为 `RELEASED`                                                                    |
| 月度费用     | 聚合账单       | 按 mock provisioned 时长 × 单价                                                        |

> **时间尺度说明**：PRD AC-04 约定 `Mock 30s ≈ 真实 30min`（1:60 缩放）；[`cloudpilot-mockup.html`](./cloudpilot-case/cloudpilot-mockup.html) 演示侧采用 5s 推进以快速演示状态机。下方代码骨架以真实生产时间为示例，落地时通过 `MOCK_TIME_SCALE` 环境变量统一调节。
...
1. **全链路仅需 5 个工件层**（4 个文件 + 1 个 OpenSpec 工件包）就能覆盖从需求到规范——访谈记录 → PRD → Mock UI → DDD 模型 → OpenSpec。
...
4. **Mock 实现并非偏离本意**——在骨架验证阶段它本就是一种主动选择。重要的是接口契约与未来换真实云 SDK 时保持不变。
...
> 以下内容浓缩自 [`./ai-native-architecture/ai-native-architecture.md`](./ai-native-architecture/ai-native-architecture.md)，聚焦"如何用三层架构把 AI 安全、可控、可度量地嵌入系统"。
...
| **交易策略**  | **不确定下多步推理** | **Agent（单主体侧唯一 Agent）** | 目标明确但路径不可枚举，需经验沉淀 |
...
> **可视化版本**：完整的交互式金字塔架构图、组件详情面板、治理平面与五角色映射表，请打开 [`./ai-native-architecture/ai-native-architecture-diagram.html`](./ai-native-architecture/ai-native-architecture-diagram.html)（点击任意 Agent / Skill / Tool 卡片可查看本质、KPI、安全约束等详细信息）。
>
> 文字概览：
>
...
判断线：Part 1 定义的 Agent (§1.1) / Skill (§1.4) / Tool (§1.2, §1.5) 边界
...
| 文档                                        | 路径                                                                                                                                   | 说明                                                          |
| :------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------ |
| AI-Native DevOps 全文                       | [`./ai-native-devops/ai-native-devops.md`](./ai-native-devops/ai-native-devops.md)                                                     | 完整方法论、治理机制、指标、仓库布局                          |
| AI-Native Architecture 全文                 | [`./ai-native-architecture/ai-native-architecture.md`](./ai-native-architecture/ai-native-architecture.md)                             | 电网交易案例驱动的三层架构推导                                |
| 订单取消变更示例演练                        | [`./ai-native-devops/ai-native-devops-sample-change-walkthrough.md`](./ai-native-devops/ai-native-devops-sample-change-walkthrough.md) | 以订单取消为例的全链路变更萃取                                |
